...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Control
        end_dist = np.hypot(
            path[-1, 0] - simulator.state.x, path[-1, 1] - simulator.state.y
        )
        if args.simulator == "basic":
            if end_dist > 10:
                next_v = 20
            ...
            r, l = simulator.wu / 2, simulator.l / 2
            next_lw = np.rad2deg(next_v / r - np.deg2rad(next_w) * l / r)
            next_rw = np.rad2deg(next_v / r + np.deg2rad(next_w) * l / r)
        ...
        img = img_path.copy()
        cv2.circle(
            img, (int(target[0]), int(target[1])), 3, (1, 0.3, 0.7), 2
        )  # target points
