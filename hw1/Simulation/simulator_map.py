...
class SimulatorMap(
    SimulatorBasic, SimulatorDifferentialDrive, SimulatorBicycle
):
    ...
    def step(self, command):
        state_next, _ = self.simulator_class.step(
            self, command, update_state=False
        )
        car_box_next = compute_car_box(
            self.car_w, self.car_f, self.car_r, state_next.pose()
        )
        collision = self.collision_detect(self.m, car_box_next)
        if collision:
            self.state.w = 0.0
            self.state.v = -0.5 * self.state.v
            state_next = self.simulator_class.step(
                self, ControlState(self.control_type, 0, 0)
            )
        else:
            self.state = state_next
            self.record.append((self.state.x, self.state.y, self.state.yaw))
            self.car_box = compute_car_box(
...
class SimulatorMapLidar(SimulatorMap):
    def __init__(
        self, simulator_class, m, lidar_params=[31, -120.0, 120.0, 250], **kargs
    ):
