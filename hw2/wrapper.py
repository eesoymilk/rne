...
class PathTrackingEnv:
    ...
    def reset(self):
        ...
        min_idx, min_dist = PathTracking.utils.search_nearest(
            self.path, position
        )
        ...
        record_path = np.concatenate(
            (self.get_record_path(-1), self.get_record_path(-1))
        )
    ...
    def step(self, action):
        ...
        record_path = np.concatenate(
            (self.get_record_path(-2), self.get_record_path(-1))
        )
        future_path = self.get_future_path(min_idx)
        ...
        reward = (
            0.8 * np.exp(-0.1 * min_dist)
            + 0.2 * np.exp(-0.1 * error_yaw * error_yaw)
            + progress_reward
        )
        self.last_idx = min_idx
        ...
        goal_dist = np.sqrt(
            (position[0] - self.path[-1, 0]) ** 2
            + (position[1] - self.path[-1, 1]) ** 2
        )
        if (
            min_idx == len(self.path) - 1
            or goal_dist < 10
    ...
    def render(self, img=None):
        ...
        cv2.circle(
            img, (int(self.target[0]), int(self.target[1])), 3, (1, 0.3, 0.7), 2
        )  # target points
