...
from Simulation.kinematic_differential_drive import (
    KinematicModelDifferentialDrive as KinematicModel,
)
...
class SimulatorDifferentialDrive(Simulator):
    ...
    def step(self, command, update_state=True):
        if command is not None:
            ...
            self.cstate.lw = (
                command.lw if command.lw is not None else self.cstate.lw
            )
            self.cstate.rw = (
                command.rw if command.rw is not None else self.cstate.rw
            )
    ...
    def render(self, img=None):
        ...
        ########## Draw Wheels ##########
        w1 = utils.rot_pos(0, self.l, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        w2 = utils.rot_pos(0, -self.l, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        # 4 Wheels
        img = utils.draw_rectangle(
            img, int(w1[0]), int(w1[1]), self.wu, self.wv, -self.state.yaw
        )
        img = utils.draw_rectangle(
            img, int(w2[0]), int(w2[1]), self.wu, self.wv, -self.state.yaw
        )
        # Axle
        img = cv2.line(
            img,
