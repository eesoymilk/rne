...
class SimulatorBicycle(Simulator):
    def __init__(
        ...
        self.car_box = utils.compute_car_box(
            self.car_w, self.car_f, self.car_r, self.state.pose()
        )
    ...
    def step(self, command, update_state=True):
        if command is not None:
            ...
            self.cstate.a = (
                command.a if command.a is not None else self.cstate.a
            )
            self.cstate.delta = (
                command.delta
                if command.delta is not None
                else self.cstate.delta
            )
    ...
    def render(self, img=None):
        ...
        for i in range(start, len(self.record) - 1):
            ...
            cv2.line(
                img,
                (int(self.record[i][0]), int(self.record[i][1])),
                (int(self.record[i + 1][0]), int(self.record[i + 1][1])),
                color,
                1,
            )
        ...
        cv2.line(
            img,
            tuple(pts1.astype(int).tolist()),
            tuple(pts2.astype(int).tolist()),
            color,
            size,
        )
        cv2.line(
        ...
        t1 = utils.rot_pos(6, 0, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        t2 = utils.rot_pos(0, 4, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        t3 = utils.rot_pos(0, -4, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        cv2.line(
            img,
        ...
        ########## Draw Wheels ##########
        w1 = utils.rot_pos(self.l, self.d, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        w2 = utils.rot_pos(self.l, -self.d, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        w3 = utils.rot_pos(0, self.d, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        w4 = utils.rot_pos(0, -self.d, -self.state.yaw) + np.array(
            (self.state.x, self.state.y)
        )
        # 4 Wheels
        img = utils.draw_rectangle(
            img,
            int(w1[0]),
            int(w1[1]),
            self.wu,
            self.wv,
            -self.state.yaw - self.cstate.delta,
        )
        img = utils.draw_rectangle(
            img,
            int(w2[0]),
        ...
        img = utils.draw_rectangle(
            img, int(w3[0]), int(w3[1]), self.wu, self.wv, -self.state.yaw
        )
        img = utils.draw_rectangle(
            img, int(w4[0]), int(w4[1]), self.wu, self.wv, -self.state.yaw
        )
        # Axle
        img = cv2.line(
            img,
            tuple(w1.astype(int).tolist()),
            tuple(w2.astype(int).tolist()),
            (0, 0, 0),
            1,
        )
        img = cv2.line(
            img,
            tuple(w3.astype(int).tolist()),
            tuple(w4.astype(int).tolist()),
            (0, 0, 0),
            1,
        )
        return img
